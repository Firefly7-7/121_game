    ...
    def options_place(self) -> None:
        ...
        def visible(contr: ControlOption) -> bool:
            ...
            if contr.dependent is None:
                return True
            elif type(contr.dependent[1]) != type(self.editing_controls[contr.dependent[0]].value):
                return False
        ...
        while self.place == "options":
            ...
            if self.control_edit is None:
                ...
                if self.text != "":
                    if self.text[-1] == "\n":
                        match = get_first_match(self.text[:-2], self.editing_controls[self.control_edit].args)
                        if match is None:
                            self.start_typing()
                            self.replace_button(
                        ...
                        match = get_first_match(self.text, self.editing_controls[self.control_edit].args)
                        if match is None:
                            self.replace_button(
                                self.editing_controls[self.control_edit].button_index,
                                self.make_text_button(
