...
class InGame(Utility):
    ...
    def in_game_place(self) -> None:
        ...
        def in_between_collision(
            ...
            :param direction: what direction coming from
            :param blocks: list of coordinate tuples
            :param player_pos: player position
            :param player_momentum: player momentum
            :return: tuple of coordinates, tuple of movement
            """
            for block in blocks:
                col = block_info.get(block)
                if col is None:
                    continue
                if col.type in SOLID_BLOCKS:
                    return position_correction(block, direction, player_pos, player_momentum)
        ...
        self.alive = True
        self.won = False
        self.message = Surface((1, 1))
        self.message.fill((255, 255, 255))
        ...
        while self.place == "in_game" and not self.won:
                ...
                for i in range(len(player_data)):
                    ...
                    *player_data[i], gravity_info = do_physics(*player_data[i])
                    in_between_player_data = [
                        (pos, (mom[0] / (frame_skip + 1), mom[1] / (frame_skip + 1))) for pos, mom, *arg in player_data
                    ]
                    if not self.alive:
                        break
            ...
            self.screen.blit(
                self.message,
                (0, 180 * 2 - self.message.get_height() / 2)
            )
            if not self.alive:
                tick = 0
                dimensions, gravity_info, block_info, link_info, player_positions = make_playable(self.level_data)
        ...
        if self.won:
            if self.after_game == "construction":
                ...
                if self.custom == 0:
                    if self.level_on == self.look_at[0]:
                        self.level_on += 1
                        self.look_at[0] += 1
