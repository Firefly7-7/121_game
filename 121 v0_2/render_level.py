...
def draw_block(block: Block, gravity: int, font: Font, scale: int = 60) -> Surface:
    ...
    barrier_dest = ((scale, 0), (scale, scale), (0, scale), (0, 0))
    for blok, grav_lock, barriers in block.barriers:
        for i in range(4):
            if barriers[i]:
                line(
                    res,
                    BARRIER_COLORS[blok][grav_lock],
                    barrier_dest[i - 1 - grav_lock * gravity],
                    barrier_dest[i - grav_lock * gravity],
                    round(scale / 4)
                )
    # links
